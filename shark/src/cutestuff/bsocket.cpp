...
class QTcpSocketSignalRelay : public QObject
{
	...
	public:
		...
		:QObject(parent)
		{
			...
			connect(sock, SIGNAL( hostFound() ), SIGNAL( hostFound() ), Qt::QueuedConnection);
			connect(sock, SIGNAL( connected() ), SIGNAL( connected() ), Qt::QueuedConnection);
			connect(sock, SIGNAL( disconnected() ), SIGNAL( disconnected() ), Qt::QueuedConnection);
			connect(sock, SIGNAL( readyRead() ), SIGNAL( readyRead() ), Qt::QueuedConnection);
			connect(sock, SIGNAL( bytesWritten(qint64) ), SIGNAL( bytesWritten(qint64) ), Qt::QueuedConnection);
			connect(sock, SIGNAL( error(QAbstractSocket::SocketError) ), SIGNAL( error(QAbstractSocket::SocketError) ), Qt::QueuedConnection);
		}
